...
export function submitBug(input: {
  kind?: string;
  message?: string;
  page?: string;
  contact?: string;
  ...
  const kind: BugReport["kind"] = input.kind === "feedback" ? "feedback" : "bug";
  const message = String(input.message ?? "").trim().slice(0, 2000);
  if (message.length < 3) return { ok: false, error: kind === "feedback" ? "Please write your feedback." : "Please describe the bug." };

  const report: BugReport = {
    id: uid(kind === "feedback" ? "fb_" : "bug_"),
    kind,
    message,
  ...
  if (WEBHOOK) {
    const label = kind === "feedback" ? "💬 **Feedback**" : "🐞 **Bug report**";
